...
int print_tally(const TallyReporter& rep) {
    ...
    if (const char* path = std::getenv("RXBENCH_TABLE")) {
        ...
        if (f) {
            ...
            // The full cheatah-bench-stamp, not a bare "generated on <date>" comment. The
            // fields scripts/bench_table.purr reads are `suite:` (which region this belongs
            // to), `commit:` + `watch:` (together: has the measured code moved since?) and
            // `publishable:`. Without them this table is invisible to the staleness gate,
            // which is how it sat outside the system while being the one genuinely generated
            // table in the tree.
            const char* commit = std::getenv("CHEATAH_BENCH_COMMIT");
            std::fprintf(f,
                         ...
                         "     statistic:    median real time per case; ratio = rival/cheatah, "
                         ...
                         "|---|--:|--:|--:|--:|--:|--:|--:|\n%s\n",
                         day, (commit != nullptr && commit[0] != '\0') ? commit : "unknown",
                         kThreshold, kMinGapNs, md.c_str());
            std::fprintf(f, "**Tally** — ");
            for (const Rival& r : rivals)
    ...
    // The REPRESENTATIVE table: a curated subset, curated in the INVOCATION.
    //
    // stdlib/regex/README.md publishes eight hand-picked cases above the full table. Picking
    // is editorial and that is fine — what is not fine is the picking living in a Markdown
    // file, where the numbers beside it cannot be regenerated. RXBENCH_ROWS carries both the
    // selection and its order, and the stamp records the spec verbatim, so the published
    // subset reproduces exactly.
    //
    //   RXBENCH_ROWS='search_status=`status=200` on a 4 MB log;search_digits=`[0-9]+` (search)'
    if (const char* rep_path = std::getenv("RXBENCH_REP_TABLE")) {
        const char* spec = std::getenv("RXBENCH_ROWS");
        std::FILE* f = std::fopen(rep_path, "w");
        if (f && spec != nullptr) {
            std::time_t now = std::time(nullptr);
            char day[16] = "unknown";
            if (std::tm* tm = std::localtime(&now)) std::strftime(day, sizeof day, "%Y-%m-%d", tm);
            ...
            // ';'-separated `case=label`, emitted in the order given — ordering is curation too.
            std::string sp(spec);
            std::size_t i = 0;
            while (i < sp.size()) {
                std::size_t semi = sp.find(';', i);
                if (semi == std::string::npos) semi = sp.size();
                const std::string entry = sp.substr(i, semi - i);
                i = semi + 1;
                if (entry.empty()) continue;
                const std::size_t eq = entry.find('=');
                const std::string key = (eq == std::string::npos) ? entry : entry.substr(0, eq);
                const std::string label = (eq == std::string::npos) ? entry : entry.substr(eq + 1);

                auto at = [&ns](const std::string& n) -> double {
                    auto it = ns.find(n);
                    return it == ns.end() ? -1.0 : it->second;
                };
                const double che = at("BM_" + key + "_cheatah");
                if (che < 0.0) {
                    // Never silently drop a curated row: a spec naming a case that did not run
                    // is a broken table, and em-dashes say so where a missing line would not.
                    std::fprintf(f, "| %s | — | — | — | — | not measured |\n", label.c_str());
                    std::fprintf(stderr, "rxbench: RXBENCH_ROWS names '%s', which produced no "
                                         "measurement\n", key.c_str());
                    continue;
                }
                auto cell = [&](const char* suffix) {
                    const double v = at("BM_" + key + suffix);
                    return v < 0.0 ? std::string("—") : fmt_ns(v);
                };
                const double re2 = at("BM_" + key + "_re2");
                char ratio[32] = "—";
                if (re2 > 0.0) std::snprintf(ratio, sizeof ratio, "%.1f\u00d7", re2 / che);
                std::fprintf(f, "| %s | %s | %s | %s | %s | %s |\n", label.c_str(),
                             fmt_ns(che).c_str(), cell("_std").c_str(), cell("_boost").c_str(),
                             cell("_re2").c_str(), ratio);
            }
            std::fclose(f);
