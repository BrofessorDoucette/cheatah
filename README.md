...
- **Batteries included.** A standard library spanning `io`, `os`, `string`,
  `math`, `time`, `datetime`, `random`, `statistics`, `hashlib`, `socket` (a small
  BSD-socket wrapper — enough to write an HTTP server in pure cheatah), and a SIMD
...
|--------|------------------|
| **Lines** | 100.00% (1496/1496) |
| **Functions** | 100.00% (268/268) |
| Regions | 95.70% |
| Branches | 88.45% |
<!-- coverage:end -->
...
The full standard-library API reference is generated with **Doxygen** into
[docs/](docs/) (open [docs/html/index.html](docs/html/index.html)). Every stdlib
function carries a Javadoc/Doxygen comment with a uniform contract:

- **`@param` / `@return`**, a one-line brief, and an **`@note`** giving the
  **runtime complexity** (Big-O) **and whether the call performs a heap
  allocation** — memory behavior is a first-class part of every signature.
- an **`@test`** link to the unit test that exercises the function.

The site is a fixed **dark, cheetah-themed** design (built on doxygen-awesome-css)
with Graphviz inheritance/collaboration/include/call/caller diagrams, a source
browser with cross-references, and conformance to **WCAG 2.1** contrast
(AA throughout, AAA for body text and accents — see
[docs/theme/ACCESSIBILITY.md](docs/theme/ACCESSIBILITY.md)). Each stdlib module
also has a short `README.md` overview. Regenerate with:

```bash
doxygen Doxyfile          # output lands in docs/html/
```
