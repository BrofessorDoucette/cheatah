...
> ⚠️ **Status: pre-alpha (v0.5.0-alpha).** cheatah runs and is heavily tested
...
**Features:** `int`/`float`/`str`/`bool`, `let` variables (with optional `: type`
annotations), arithmetic + `**` power, comparisons, `and`/`or`/`not`,
`if`/`elif`/`else`, `match`/`case`, `while`, `for … in range(…)`/containers with
`break`/`continue`, `fn` functions (with recursion), `struct` records,
`list`/`dict`/`array` collections with indexing, **slicing** (`a[i:j]`), negative
...
|--------|------------------|
| **Lines** | 100.00% (2279/2279) |
| **Functions** | 100.00% (340/340) |
| Regions | 96.15% |
| Branches | 89.64% |
<!-- coverage:end -->
...
The full standard-library API reference is built into [docs/](docs/) (open
...
accessible contrast (see [docs/theme/ACCESSIBILITY.md](docs/theme/ACCESSIBILITY.md)).
Each stdlib module also has a short `README.md` overview. Regenerate with:
