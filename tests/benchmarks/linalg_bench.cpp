// Micro-benchmarks for cheatah::linalg (exercised by tests/linalg/smoke_test.cpp).
#include "linalg.hpp"
...
void BM_Version(benchmark::State& state) {
    for (auto _ : state) {
        benchmark::DoNotOptimize(cheatah::linalg::version());
    }
}
...
void BM_Dot(benchmark::State& state) {
    ...
    for (auto _ : state) {
        benchmark::DoNotOptimize(cheatah::linalg::dot(a, b));
    }
