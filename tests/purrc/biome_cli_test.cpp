...
TEST_F(BiomeCli, StandardsListsKnownSets) {
  ...
  expect_has(out, "* 0.6.2-alpha  (current, released 2026-08-15)");
  expect_has(out, "0.6.0-alpha  (supported, released 2026-08-14)");
  expect_has(out, "0.4.0-alpha  (supported, released 2026-08-13)");
  expect_has(out, "cheatah             v1.11.6-alpha");  // ljust(name, 20) columns
...
// `biome init` pins the newest standard: cheatah.toml gets `standard = "0.6.2-alpha"`
...
TEST_F(BiomeCli, AddStandardMemberExtension) {
  ...
  expect_has(out, "(from Biome Standard 0.6.2-alpha)");
  const std::string toml = e2e::read_file(proj + "/cheatah.toml");
...
TEST_F(BiomeCli, AddNonMemberExtensionRefused) {
  ...
  const auto at = manifest.find("standard = \"0.6.2-alpha\"");
  ASSERT_NE(at, std::string::npos);
  manifest.replace(at, std::string("standard = \"0.6.2-alpha\"").size(),
...
TEST_F(BiomeCli, ListShowsMembershipAndStandard) {
  ...
  expect_has(out, "Biome Standard 0.6.2-alpha");
...
TEST_F(BiomeCli, LegacyManifestDefaultsStandard) {
  ...
  const std::string toml = e2e::read_file(dir + "/cheatah.toml");
  expect_has(toml, "standard = \"0.6.2-alpha\"");  // written on the first save
