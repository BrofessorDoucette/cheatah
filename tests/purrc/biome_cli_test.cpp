...
TEST_F(BiomeCli, StandardsListsKnownSets) {
  ...
  expect_has(out, "* 0.6.5-alpha  (current, released 2026-08-28)");
  // The predecessor's status flips on release — the append-only record's one mutable field.
  expect_has(out, "0.6.4-alpha  (supported, released 2026-08-28)");
  expect_has(out, "0.6.2-alpha  (supported, released 2026-08-15)");
  expect_has(out, "0.6.0-alpha  (supported, released 2026-08-14)");
  expect_has(out, "0.4.0-alpha  (supported, released 2026-08-13)");
  expect_has(out, "cheatah             v1.11.9-alpha");  // ljust(name, 20) columns
  expect_has(out, "cheatah-gpu         v0.5.1-alpha");
  expect_has(out, "cheatah-gpu-linalg  v0.4.5-alpha");
  expect_has(out, "cheatah-plot        v0.1.1-alpha");
...
// `biome init` pins the newest standard: cheatah.toml gets `standard = "0.6.5-alpha"`
...
TEST_F(BiomeCli, AddStandardMemberExtension) {
  ...
  expect_has(out, "(from Biome Standard 0.6.5-alpha)");
  const std::string toml = e2e::read_file(proj + "/cheatah.toml");
...
// written — proven against 0.4.0-alpha, the last standard before cheatah-plot joined the set.
// (Exit code stays 0 — the run() wart — so assert text only.)
TEST_F(BiomeCli, AddNonMemberExtensionRefused) {
  ...
  const auto at = manifest.find("standard = \"0.6.5-alpha\"");
  ASSERT_NE(at, std::string::npos);
  manifest.replace(at, std::string("standard = \"0.6.5-alpha\"").size(),
...
// The three members that joined at 0.5.0-alpha now ADD cleanly, each pinned at the
// standard's tag in the generated CMake.
TEST_F(BiomeCli, AddNewStandardMembersSucceed) {
  const std::string proj = init_project("add_new_members");
  biome(proj, "add cheatah-gpu-linalg");
  biome(proj, "add cheatah-plot");
  biome(proj, "add cheatah-space");
  const std::string cml = e2e::read_file(proj + "/CMakeLists.txt");
  expect_has(cml,
             "CPMAddPackage(NAME cheatah-gpu-linalg GITHUB_REPOSITORY "
             "BrofessorDoucette/cheatah-gpu-linalg GIT_TAG v0.4.5-alpha)");
  expect_has(cml,
             "CPMAddPackage(NAME cheatah-plot GITHUB_REPOSITORY BrofessorDoucette/cheatah-plot "
             "GIT_TAG v0.1.1-alpha)");
  expect_has(cml,
             "CPMAddPackage(NAME cheatah-space GITHUB_REPOSITORY BrofessorDoucette/cheatah-space "
...
TEST_F(BiomeCli, ListShowsMembershipAndStandard) {
  ...
  expect_has(out, "Biome Standard 0.6.5-alpha");
  expect_has(out, "* cheatah-gpu         v0.5.1-alpha");       // added member: marked + tagged
  expect_has(out, "  cheatah-gpu-linalg  v0.4.5-alpha");       // members, not yet added
  expect_has(out, "  cheatah-plot        v0.1.1-alpha");
  expect_has(out, "  cheatah-space       v0.1.0-alpha");
}
...
TEST_F(BiomeCli, LegacyManifestDefaultsStandard) {
  ...
  const std::string toml = e2e::read_file(dir + "/cheatah.toml");
  expect_has(toml, "standard = \"0.6.5-alpha\"");  // written on the first save
