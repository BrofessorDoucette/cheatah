...
| `p256` ECDSA **signing** | branchy scalar-mul / Montgomery reduction is not fully constant-time | **Documented limitation** — signing is for offline/tooling use (JWTs); a co-resident side-channel attacker on an *online* signer could leak nonce bits. Verify-only paths (TLS) are unaffected. |
