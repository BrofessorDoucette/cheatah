...
def build_sidebar(compounds: dict[str, Compound]) -> str:
    ...
    # The Transpiler guide is generated by us (not a Doxygen @page), so inject it into the
    # reading order by hand — right after getting-started / porting, before performance.
    links = []
    for c in guides:
