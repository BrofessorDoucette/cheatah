#!/usr/bin/env python3
"""cheatah docs site generator — HISTORICAL REFERENCE, not a benchmark baseline.

The site is generated by the pure-cheatah port of this file
(docs/gen-cheatah/gen.purr, run by docs/build-docs.sh). This Python original was
kept as a parity reference, and that parity is GONE: gen.purr writes 292 pages to
this file's 252, and every shared page differs. It knows nothing about the package
switcher, the Biome Standard page, or the extension subsites — it still renders
cheatah-gpu, cheatah-plot and the rest as flat namespaces in the main sidebar.

Nothing runs this file and no gate compares the two, so do NOT cite the pair as a
like-for-like cheatah-vs-CPython measurement. The honest doc-render comparison is
docs/gen-cheatah/gen_bench.purr vs gen_bench.py — two small programs written to do
identical work, checked against each other's output.

Restoring the comparison means porting this file forward to subsites and adding a
diff -r check to the QA gate, in that order.
...
<header class="topbar">
  <div class="topbar-inner">
    ...
    <div class="topbar-right">
      <a class="repo" href="https://github.com/BrofessorDoucette/cheatah">Source <span aria-hidden="true">↗</span></a>
