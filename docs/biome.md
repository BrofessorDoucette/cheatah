...
- **If you know Python's pip + venv:** biome is *both*, fused into the build, with <b>no
  `venv` to create or activate</b>. `cheatah.toml` lists your dependencies the way
  `requirements.txt` / `pyproject.toml` does; biome fetches each one **locally, into this
  project's own build folder**, and CMake compiles against exactly those copies. There is no
  global, mutable `site-packages` and no "which include path won?" ambiguity — the isolation
  is hermetic by construction rather than something you switch on.
...
A cheatah program becomes native code, so building one means fetching the toolchain
(`purrc`, the runtime, the standard library) plus every extension you use, and wiring each
onto the compiler's search path. biome does that hand-work. All it really does is
(1) regenerate `CMakeLists.txt` from `cheatah.toml` and (2) invoke `cmake` — split across
`biome configure` (the CMake configure, where CPM downloads each pinned dependency into
your build folder) and `biome build` (the CMake build). Everything after that — fetching
the toolchain, compiling your `.purr`, linking — is plain CMake, so **anything you already
know about CMake still applies**, and the generated `CMakeLists.txt` is ordinary CMake you
could run yourself.

## biome is itself a cheatah program

biome is written *in cheatah* ([biome.purr](https://github.com/BrofessorDoucette/cheatah/blob/main/pkg-manager/biome.purr)).
`purrc` compiles it to a loadable module (`biome.so`) and a small native **launcher** named
`biome` runs that module through the cheatah runtime, built with the same
`cheatah_add_program` helper your projects use. At runtime biome shells out to `cmake`; it
has no build dependency of its own beyond the toolchain it ships with.
...
biome standards [--emit-toml <ver>]
                      list the Biome Standards; --emit-toml prints one as its standards/*.toml
...
That is the **complete** command surface:
...
- **Configure and build are separate** (as in plain CMake). `biome configure` runs the CMake
  ...
  rebuild). `--clean-first` and `standards --emit-toml <ver>` are the only flags biome adds.
...
generated `CMakeLists.txt` — `biome configure`, `biome add` and `biome remove` rewrite it from
the manifest, so the manifest is the single source of truth and the two never drift apart.
...
[cheatah]
standard = "0.6.5-alpha"    # the Biome Standard — ONE version pinning the whole tested set
...
[components]
cheatah = "v1.11.9-alpha"
cheatah-gpu = "v0.5.1-alpha"
cheatah-gpu-linalg = "v0.4.5-alpha"
cheatah-plot = "v0.1.1-alpha"
cheatah-space = "v0.1.0-alpha"
```
...
- **Nothing is ever stranded** — every standard's member sources stay obtainable
  ...
  documentation generators — the full API contract (`@param`/`@return`/`@complexity`/
  `@alloc`/`@test`) lives *in the source headers*, not only on this site — a project can
  rebuild these documentation pages from the sources alone.
...
| `cheatah-gpu-linalg` | GPU linear algebra (the `linalg` device backend) |
...
1. adds `cheatah-gpu = "v0.5.1-alpha"` (the tag resolved from your standard) to the
...
- <b>`biome configure`</b> regenerates `CMakeLists.txt` from the manifest, then runs
  ...
  2. `CPMAddPackage(NAME cheatah … GIT_TAG v1.11.9-alpha)` — **fetches** the cheatah toolchain
...
A cheatah pulled in as a sub-project builds without its test suite — tests are behind
`CHEATAH_BUILD_TESTS`, off unless a preset or `-D` turns it on.
...
runtime, driven entirely by CMake/CPM. Biome Standard 0.6.5-alpha is the current one and has
**five members**, each at a published release tag: the cheatah toolchain (`v1.11.9-alpha`),
`cheatah-gpu` (`v0.5.1-alpha`), `cheatah-gpu-linalg` (`v0.4.5-alpha`), `cheatah-plot`
(`v0.1.1-alpha`) and `cheatah-space` (`v0.1.0-alpha`). Every standard is a working
combination of all five; `scripts/test-standard-e2e.sh` proves the current one from an empty
directory against the real GitHub tags.
...
for e in cheatah-gpu cheatah-gpu-linalg cheatah-plot cheatah-space; do biome add $e; done
```
