...
- **Memory-safe generated code by construction.** Codegen emits value types, STL
  ...
  arithmetic** — so use-after-free and double-free are off the table for compiler-written
  code. (The one exception is the `cpp { … }` escape hatch — see below.)
...
- **A known TOCTOU window on module load.** A small time-of-check/time-of-use gap
  sits between the runtime's file checks and the `dlopen`. It is **acceptable under
  full trust** but is no defense against an attacker who can swap files underneath you.
